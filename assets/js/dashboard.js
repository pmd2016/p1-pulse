    ...
    const DashboardManager = {
        ...
        async loadElectricityData() {
            try {
                ...
                const current = await window.P1API.getCurrentData();
                if (current) {
                    this.updateElement('elec-current-power', this.formatPower(current.currentPower || 0));
                }

                // Get today's totals (use hours period with 24h zoom for today)
        ...
        async loadSolarData() {
            try {
                ...
                if (current && current.power !== undefined) {
                    this.updateElement('solar-current-power', this.formatPower(current.power));
                }
